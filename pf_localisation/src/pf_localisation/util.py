#!/usr/bin/env python
""" A file with useful utility functions, usually related to quaternions. 
...
def timed(fn):
    ...
    Returns:
        ret: The return value from fn.

    """
    def timed(*args,  **kwargs):
        t = time.time()
        print("[" + str(fn) + str(__name__) + "]Start: " +  str(t))
        ret =  fn(*args, **kwargs)
        print("[" + str(fn) + str(__name__) + "]End:" + 
              str(time.time()) + " = = = " + str(time.time() - t))
        return ret
    return timed

def rotateQuaternion(q_orig, yaw):
    """ Converts z-rotation into ROS Quaternion.
