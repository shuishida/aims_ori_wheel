...
Author: Someone at the University of Birmingham
...
class SensorModel(object):
    ...
    def calc_map_range(self, ox, oy, oa):
        """ Predict the range of the laser.
    ...
    def get_weight(self, scan, pose):
        ...
        for i, obs_bearing in self.reading_points:
            ...
            map_range = self.calc_map_range(pose.position.x, pose.position.y,
                                            getHeading(pose.orientation) + 
                                            obs_bearing)
            pz = self.predict(obs_range, map_range)
            p += pz * pz * pz  # Cube prob: reduce low-probability particles
    ...
    def predict(self, obs_range, map_range):
        ...
        # This code is very dumb but so is the robot's laptop
        if (pz >= 0.999999):
            #print("pz was %2.9f" % pz)
            pz = 0.999999
